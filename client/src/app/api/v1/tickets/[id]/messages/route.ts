...
export async function POST(
  ...
  try {
    ...
    let holds = await cosmosDbService.getCacheData(holdsKey, authUser.institution_id) || [];
    ...
    if (normalizedContent.includes("hold") || normalizedContent.includes("bakit may") || normalizedContent.includes("sagabal")) {
      ...
      if (activeHoldsCount === 0) {
        ...
        const holdLines = holds
          .map((h: any, i: number) => `${i + 1}. **${h.type} Hold** (${h.status}): ${h.reason}\n   *Lunas:* ${h.resolution_steps}`)
          .join("\n\n");
      ...
      const financialHold = holds.find((h: any) => h.id === "hold-financial" && h.status === "Active");
      if (financialHold) {
    ...
    } else if (normalizedContent.includes("human") || normalizedContent.includes("agent") || normalizedContent.includes("makausap") || normalizedContent.includes("jay") || normalizedContent.includes("staff")) {
      toolCalls.push("EscalateToHuman");
      
      // Update ticket status to Pending Agent
      await cosmosDbService.updateConversationStatus(conversationId, authUser.institution_id, "Pending Agent", "agent-jay-oid");
      ...
      const handoffDoc: HandoffDoc = {
        ...
        agent_id: "agent-jay-oid",
        resolved_at: undefined,
      };
      await cosmosDbService.createHandoff(handoffDoc);

      aiContent = "Nauunawaan ko po. Inilipat ko na ang usapang ito sa ating student services desk agent na si **Jay**. Kasama po sa inilipat ko ang summary ng holds at mga naisagawa ko upang hindi mo na kailangang ulitin ang kwento mo. \n\nMangyaring maghintay ng ilang sandali habang sumasagot si Jay...";
    } else {
      aiContent = `Salamat sa mensahe, ${authUser.name || "Student"}. Maaari mo akong tanungin tungkol sa iyong holds ('bakit may hold ako?'), balanse sa tuition, UniFAST scholarship deadlines, o mag-request na makipag-usap sa support agent (Jay).`;
    }
