...
The documentation suite in the [docs/](docs) folder uses the **Formal Markdown Document (FMD)** framework. The index is located at [docs/index.md](docs/index.md).
...
| :--- | :--- | :--- |
| **Documentation Index** | [docs/index.md](docs/index.md) | The master directory. Contains the Change Log, Incident Log, and the session Health Check list. |
| **Business Requirements** | [docs/brd-archon.md](docs/brd-archon.md) | Business metrics, targets, financial projections, and operational objectives (IDs: `BRD-M#`). |
| **Product Requirements** | [docs/prd-archon.md](docs/prd-archon.md) | Core user stories, priority features, language requirements, and acceptance criteria (IDs: `PRD-F#`). |
| **Design System** | [docs/dsd-archon.md](docs/dsd-archon.md) | Visual tokens, CSS variables, interactive states, accessibility rules, and Next.js frontend UI specifications. |
| **System Design** | [docs/sdd-archon.md](docs/sdd-archon.md) | C4 architecture diagrams, Azure infrastructure specifications, schema descriptions, and system integrations. |
| **QA & Test Plan** | [docs/qad-archon.md](docs/qad-archon.md) | Test suites, Playwright E2E targets, Jest unit test structures, and AI Foundry Prompt evaluation metrics. |
| **Subagents Roster** | [docs/sad-archon.md](docs/sad-archon.md) | Defines the roster of the 4 specialist agents (`SAD-A1` to `SAD-A4`) and delegation loops. |
| **Build Guide / Stack Rules** | [docs/build-archon.md](docs/build-archon.md) | Pinned stack versions, deprecation notices, repo directory layouts, and code templates. |
| **Compliance & Legal** | [docs/clr-archon.md](docs/clr-archon.md) | Data Inventory, PH DPA 2012 / FERPA rules, retention schedules, and PII minimization protocols. |
| **Go-To-Market** | [docs/gtm-archon.md](docs/gtm-archon.md) | Launch timelines, user onboarding stages, pricing tiers, and success metrics mapping. |
| **Ops & Observability** | [docs/ops-archon.md](docs/ops-archon.md) | Monitoring setup, Application Insights metrics, alerts configuration, and incident response matrices. |

### Technical RFCs

RFCs specify technical designs for major capabilities. They must align strictly with the SDD:
- **RFC-001 (Agent Orchestration):** [rfc-archon-agentic-orchestration.md](docs/rfc-archon-agentic-orchestration.md) — Azure AI Foundry Agents + tool-calling setup.
- **RFC-002 (Human Handoff):** [rfc-archon-human-handoff.md](docs/rfc-archon-human-handoff.md) — Preserving context, Escalation JSON packets, and wrap-up loops.
- **RFC-003 (University Adapters):** [rfc-archon-university-adapters.md](docs/rfc-archon-university-adapters.md) — Gateway mapping pattern for legacy registrar/bursar endpoints.
- **RFC-004 (M365 Integration):** [rfc-archon-m365-integration.md](docs/rfc-archon-m365-integration.md) — Entra ID SSO, Graph API, Teams adaptive cards, and Power Automate schedulers.
...
### Onboarding Steps:
1. **Initialize Context:** Open and review [docs/index.md](docs/index.md) to parse the current versions, status, and active change logs.
2. **Review Tech Stack:** Review [docs/build-archon.md](docs/build-archon.md) to extract current framework versions, directory paths, and pattern conventions.
...
Add a row at the top of the **Change Log** table in [docs/index.md](docs/index.md) using this schema:
```markdown
...
- [ ] **Stack Alignment:** Pinned stack versions in code match the table in [docs/build-archon.md](docs/build-archon.md).
...
- [ ] **PII Minimization Verification:** Ensure all data operations respect the boundaries of the [docs/clr-archon.md](docs/clr-archon.md):
...
- [ ] **Subagent Alignment:** Any new automation or tooling fits within the 4 specialist agent descriptions in [docs/sad-archon.md](docs/sad-archon.md) (`archon-feature-builder`, `archon-adapter-scaffolder`, `archon-test-runner`, `archon-compliance-checker`). No additional agent files are created.
...
1. File a postmortem record in the **Incident Log** in [docs/index.md](docs/index.md).
...
3. Do not mark the postmortem action items as complete until code modifications are verified by the [docs/sad-archon.md](docs/sad-archon.md) Test Runner agent.
